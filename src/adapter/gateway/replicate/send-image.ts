...
export const buildSendImage = ({ replicateBalancer }: Params): SendImage => {
  return async ({ message, settings, model }) => {
    try {
      ...
      if (
        error.message?.includes('All generated images contained NSFW content.') ||
        error.message?.includes('Error generating image: NSFW content detected')
      ) {
