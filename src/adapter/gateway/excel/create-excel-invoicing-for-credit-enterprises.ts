...
export const buildCreateExcelInvoicingForCreditEnterprises =
  (): CreateExcelInvoicingForCreditEnterprises => {
    return async ({ params }: CreateExcelInvoicingForCreditEnterprisesParams) => {
      ...
      wsData.push([
        ...
        'Дата договора',
        'Курс',
        ...params[0].creditedTokensForDates.map((month) => month.month),
      ])
      ...
      for (const enterprise of params) {
        ...
        wsData.push([
          ...
          rubs_per_million_caps,
          ...creditedTokensForDates.map((date) =>
            date.creditedTokens ? Math.trunc(Number(date.creditedTokens) * rate) : 0,
          ),
        ])
      }
