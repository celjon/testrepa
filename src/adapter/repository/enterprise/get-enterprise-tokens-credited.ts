...
export type GetEnterpriseTokensCredited = (
  month: string,
  year: string,
) => Promise<
  Array<{
...
export const buildGetEnterpriseTokensCredited = ({ db }: Params): GetEnterpriseTokensCredited => {
  return async (month: string, year: string) => {
    const now = performance.now()
    logMemoryUsage(`Start GetEnterpriseTokensCredited`)
  ...
  EXTRACT(YEAR FROM t.created_at) AS year
    ...
    logMemoryUsage(`End GetEnterpriseTokensCredited ${performance.now() - now}ms`)
