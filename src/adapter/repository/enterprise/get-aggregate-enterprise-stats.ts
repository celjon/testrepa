...
export type GetEnterpriseStats = (params: { enterpriseId: string; from: Date; to: Date; search?: string }) => Promise<{
  employees: Array<{
    email: string | null
    id: string
    tg_id: string | null
...
export const buildGetEnterpriseStats = ({ db }: Params): GetEnterpriseStats => {
  return async ({ enterpriseId, from, to, search }) => {
    ...
    logMemoryUsage(`Start getAggregatedEnterpriseStats, for enterpriseId: ${enterpriseId}, from: ${from}, to: ${to}`)
    ...
    console.log(normalizedDateFrom, normalizedDateTo)
    ...
    const totalEnterpriseTokensUsed = employees.reduce((total, employee) => total + employee.usedTokens, 0n)
