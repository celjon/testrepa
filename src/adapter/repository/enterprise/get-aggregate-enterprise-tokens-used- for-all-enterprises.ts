...
export type GetAggregateEnterpriseTokensUsedForAllEnterprises = (
  month?: string,
  year?: string
) => Promise<Array<{ name: string; creator: string; agreement_conclusion_date: string; usedTokens: bigint; month: string; year: string }>>

export const buildGetAggregateEnterpriseTokensUsedForAllEnterprises = ({
  db
}: Params): GetAggregateEnterpriseTokensUsedForAllEnterprises => {
  return async (month?: string, year?: string) => {
    const now = performance.now()
    logMemoryUsage(`Start GetAggregateEnterpriseTokensUsed`)
    ...
    logMemoryUsage(`End GetAggregateEnterpriseTokensUsed ${performance.now() - now}ms`)
