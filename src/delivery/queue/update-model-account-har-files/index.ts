...
export const startUpdateModelAccountHARFilesQueue = async (params: Params, queues: Queues, createWorker: CreateQueueWorker) => {
    ...
    async (job: Job<UpdateModelAccountHARFileJob>) => {
      try {
        ...
        if (
          ...
          modelAccount.g4f_har_file_updated_at.getTime() + timeBetweenUpdatesMs > new Date().getTime()
        ) {
          return
        }

        const response = await params.model.autoUpdateAccountHARFile({ accountId: job.data.modelAccountId })
