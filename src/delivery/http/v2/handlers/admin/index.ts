...
import { buildPlanAddModel, PlanAddModel } from './plan/add-model'
import { buildPlanRemoveModel, PlanRemoveModel } from './plan/remove-model'
import {
  buildCreateReferralTemplate,
  CreateReferralTemplate,
} from '@/delivery/http/v2/handlers/admin/referral-template/create'
import {
  buildListReferralTemplate,
  ListReferralTemplate,
} from '@/delivery/http/v2/handlers/admin/referral-template/list'
import {
  buildDeleteReferralTemplate,
  DeleteReferralTemplate,
} from '@/delivery/http/v2/handlers/admin/referral-template/delete'
...
import { buildPlanSetDefaultModel, PlanSetDefaultModel } from './plan/set-default-model'
import { buildPlanUnsetDefaultModel, PlanUnsetDefaultModel } from './plan/unset-default-model'
import { buildPlatformTokens, PlatformTokens } from './statistics/platform-tokens'
import { buildGetTokensByModel, GetTokensByModel } from './statistics/get-tokens-by-model'
import { buildAdminRules } from './rules'
import { Middlewares } from '../../middlewares'
import {
  buildGetProductUsageReport,
  GetProductUsageReport,
} from './statistics/get-product-usage-report'

type Params = Pick<
  DeliveryParams,
  'user' | 'plan' | 'referralTemplate' | 'model' | 'transaction' | 'statistics' | 'middlewares'
>
...
const buildRegisterRoutes = (methods: AdminMethods, middlewares: Middlewares) => {
  return (root: Express.Router) => {
    ...
    namespace.get(
      '/user/:id/transaction/list',
      rules.transactionListRules,
      createRouteHandler(methods.transaction.list),
    )

    namespace.post(
      '/plan/:id/model/add',
      rules.planModelRules,
      createRouteHandler(methods.plan.addModel),
    )
    namespace.post(
      '/plan/:id/model/set-default',
      rules.modelSetDefaultRules,
      createRouteHandler(methods.plan.setDefaultModel),
    )
    namespace.post(
      '/plan/:id/model/unset-default',
      rules.unsetDefaultModelRules,
      createRouteHandler(methods.plan.unsetDefaultModel),
    )
    namespace.patch('/plan/:id', rules.updatePlanRules, createRouteHandler(methods.plan.update))
    namespace.post(
      '/plan/:id/model/remove',
      rules.planModelRules,
      createRouteHandler(methods.plan.removeModel),
    )

    namespace.get(
      '/referral-template/list',
      rules.listReferralTemplateRules,
      createRouteHandler(methods.referralTemplate.list),
    )
    namespace.post(
      '/referral-template',
      rules.createReferralTemplateRules,
      createRouteHandler(methods.referralTemplate.create),
    )
    namespace.delete(
      '/referral-template/:id',
      rules.deleteReferralTemplateRules,
      createRouteHandler(methods.referralTemplate.delete),
    )

    namespace.post(
      '/model/enable',
      rules.modelEnableDisableRules,
      createRouteHandler(methods.model.enable),
    )
    namespace.post(
      '/model/disable',
      rules.modelEnableDisableRules,
      createRouteHandler(methods.model.disable),
    )
    namespace.patch('/model', rules.updateModelRules, createRouteHandler(methods.model.update))

    namespace.get(
      '/statistics/platform-tokens',
      rules.platformTokensRules,
      createRouteHandler(methods.statistics.platformTokens),
    )
    namespace.get(
      '/statistics/tokens-by-model',
      rules.getTokensByModelRules,
      createRouteHandler(methods.statistics.getTokensByModel),
    )
    namespace.get(
      '/statistics/product-usage',
      rules.getTokensByModelRules,
      createRouteHandler(methods.statistics.getProductUsageReport),
    )
