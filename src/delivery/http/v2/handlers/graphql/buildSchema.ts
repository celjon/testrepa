...
import { buildSchemaSync, MiddlewareInterface, NextFn, registerEnumType, ResolverData } from 'type-graphql'
...
import { BaseError, ForbiddenError, InvalidDataError, NotFoundError, UnauthorizedError } from '@/domain/errors'
...
export const buildSchema = (resolvers: Resolvers) => {
  ...
  const schema = buildSchemaSync({
    resolvers: [AuthResolver, ChatResolver, MessageResolver, GroupResolver, ModelResolver, ShortcutResolver, PlanResolver],
    container: Container,
    scalarsMap: [
      {
        type: Date,
