...
export type BlockForbiddenCountriesMiddleware = (
  blockedCountries: string[],
) => (req: Request, res: Response, next: NextFunction) => void

export const buildBlockForbiddenCountries = ({
  geoGateway,
}: Params): BlockForbiddenCountriesMiddleware => {
      ...
      try {
        ...
        if (
          ...
          blockedCountries.includes(
            ipMetadata.country?.iso_code ?? ipMetadata.registered_country?.iso_code ?? '',
          )
        ) {
