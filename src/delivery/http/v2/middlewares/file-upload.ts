...
export const buildFileUploadMiddleware = (): FileUploadMiddleware => {
  return ({
    saveFiles = true,
    maxSize = cfg.upload_constraints.max_text_file_size,
    getMaxSizeInBytes,
  } = {}) => {
    ...
    const minioStorage: StorageEngine = {
      _handleFile: async (req, file, cb) => {
        ...
        if (
          ...
          file.originalname.match(/.jpg$/i) ||
          file.originalname.match(/.jpeg$/i)
        ) {
        ...
        try {
          ...
          const fileExtNames =
            /jpeg|jpg|png|svg|docx|xlsx|pdf|flac|m4a|mp3|mp4|mpeg|mpga|oga|ogg|wav|webm|avi|wmv/i
          ...
          if (getMaxSizeInBytes) {
            ...
            if (
              buffer.length === 0 ||
              !(fileExtNames.test(originalExtname.toLowerCase()) || isTextBuffer(buffer))
            ) {
            ...
            if (/jpeg|jpg|png/i.test(originalExtname)) {
              ...
              /flac|m4a|mp3|mp4|mpeg|mpga|oga|ogg|wav|webm|avi|wmv/i.test(originalExtname)
            ) {
