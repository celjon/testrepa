...
export const buildAuthRequired = ({
  developerKeyRepository,
  userRepository,
}: Params): AuthRequiredMiddleware => {
  return ({ developerOnly = false, adminOnly = false, required = true } = {}) => {
    return async (req, res, next) => {
      ...
      let key: IDeveloperKey | null = null
      if (tokenPayload?.isDeveloper) {
        key = await developerKeyRepository.get({
          where: {
            key: token,
          },
        })
      ...
      req.user = {
        ...
        developerKeyId: key?.id ?? undefined,
      }
      next()
    }
  }
