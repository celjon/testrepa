    ...
    imaginepro: z.object({
      ...
      balancer: z
        .object({
          ...
          logs: z.boolean().default(true),
          logLevel: z.enum(['low', 'high']).default('low')
        })
        .default({ disabled: false, logs: true, logLevel: 'low' })
    }),
    midjourney: z.object({
      id: z.string(),
      name: z.string(),
...
export const workerCount = process.env.WORKER_COUNT ? parseInt(process.env.WORKER_COUNT) : os.cpus().length - 1
