...
const generateKEKSalt = async (): Promise<Buffer> =>
  Buffer.from(await generateRandomBytes(constants.SALT_BYTE_LEN))
...
const getKeyFromString = async (str: string) => {
  ...
  const key = await crypto.subtle.importKey('raw', hash, { name: constants.ENCRYPTION }, true, [
    'encrypt',
    'decrypt',
  ])
...
const generateDEK = async (): Promise<Buffer> =>
  Buffer.from(await generateRandomBytes(constants.DEK_BYTE_LEN))

const encryptDEK = async ({ kek, dek }: EncryptDEKParams): Promise<Buffer> => {
  try {
    const key = await crypto.subtle.importKey(
      'raw',
      Buffer.from(kek, BUFFER_STRING_REPR),
