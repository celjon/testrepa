...
export async function get_space_hardware(space_id: string, token: `hf_${string}`) {
  const headers: { Authorization?: string } = {}
  if (token) {
    headers.Authorization = `Bearer ${token}`
  }
...
export async function set_space_timeout(space_id: string, timeout: number, token: `hf_${string}`) {
  const headers: { Authorization?: string } = {}
  if (token) {
    headers.Authorization = `Bearer ${token}`
  }
