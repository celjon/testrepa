    ...
    return new Promise((res) => {
      const createClient = async () => {
        ...
        async function view_api(config?: Config): Promise<ApiInfo<JsApiData>> {
          ...
          let api_info = (await response.json()) as ApiInfo<ApiData> | { api: ApiInfo<ApiData> }
          if ('api' in api_info) {
...
function normalise_file(file: any, root: any, root_url: any): Array<FileData> | FileData | null {
...
function get_description(type: { type: any; description: string }, serializer: string) {
...
function transform_api_info(api_info: ApiInfo<ApiData>, config: Config, api_map: Record<string, number>): ApiInfo<JsApiData> {
  ...
  for (key in api_info) {
    ...
    for (const endpoint in cat) {
      ...
      // @ts-ignore
      new_data[key][endpoint].type = config.dependencies[dep_index as any].types
      // @ts-ignore
...
async function get_jwt(space: string, token: `hf_${string}`): Promise<string | false> {
  try {
