  ...
  return async () => {
    ...
    if (queues.length === 1) queueIndex = 0
      ...
      if (!lastUsedQueueId) queueIndex = 0
        ...
        queueIndex = (queues.findIndex((queue) => queue.id === lastUsedQueueId) + 1) % queues.length
    }
    ...
    if (!account) {
      await tgNotificationBotGateway.send(
        `MJ: Не найден активный аккаунт в очереди ${queues[queueIndex]?.name}`,
        TgBotParseMode.HTML,
      )
      throw new NotFoundError({
        code: 'MIDJOURNEY_ACTIVE_ACCOUNT_NOT_FOUND',
        message: `Add or active at least one account for Midjourney queue ${queues[queueIndex]?.name}`,
      })
    }
