...
export const buildSwitchNext = ({ modelAccountRepository, modelAccountQueueRepository }: Params): SwitchNext => {
  ...
  const handleMultipleAccounts = async (accounts: IModelAccount[], currentStatus?: ModelAccountStatus): Promise<IModelAccount> => {
    ...
    const allowedStatuses: ModelAccountStatus[] = [ModelAccountStatus.FAST, ModelAccountStatus.CREATED]
  ...
  return async ({ accountQueue, currentStatus, recalculateTimeOnly = false }) => {
    ...
    const nextAccount = accountCount === 1 ? await handleSingleAccount(accounts[0]) : await handleMultipleAccounts(accounts, currentStatus)
