    ...
    let defaultModel =
      plan.models.find(
        ({ model, is_default_model }) =>
          !model.disabled && !model.disabledWeb && model.parent_id === parentId && is_default_model,
      ) ?? null

    if (!defaultModel) {
      defaultModel =
        plan.models.find(
          ({ model }) => !model.disabled && !model.disabledWeb && model.parent_id === parentId,
        ) ?? null
    }
