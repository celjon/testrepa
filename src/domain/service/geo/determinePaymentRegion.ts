...
export const buildDeterminePaymentRegion = ({ geoGateway }: Params): DeterminePaymentRegion => {
  return async ({ ip }) => {
    ...
    if (ipMetadata?.country?.iso_code === 'RU' || ipMetadata?.registered_country?.iso_code === 'RU') {
      return Region.RU
    }

    if (ipMetadata?.country?.iso_code === 'KZ' || ipMetadata?.registered_country?.iso_code === 'KZ') {
      return Region.KZ
    }
