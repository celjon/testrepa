    ...
    return [
      {
        ...
        data: childModels
          .map((imageModel) => {
            const isAllowed: boolean = currentPlanModels.some(
              ({ model_id }) => model_id === imageModel.id,
            )

            let allowedPlanType: TPlanType | null
            if (!isAllowed) {
              allowedPlanType =
                allPlanModels.find(({ model_id }) => model_id === imageModel.id)?.plan.type ?? null
            } else {
            ...
            return {
              ...
              is_default:
                isAllowed &&
                allPlanModels.some(
                  (planModel) => planModel.model_id === imageModel.id && planModel.is_default_model,
                ),
            }
          })
          .sort((imageModelA, imageModelB) => (imageModelA.order ?? 0) - (imageModelB.order ?? 0))
          .sort(
            (imageModelA, imageModelB) =>
              Number(!imageModelA.is_allowed) - Number(!imageModelB.is_allowed),
          ),
      },
    ]
  }
