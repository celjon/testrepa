...
import { getPlatformDisabledKey, isReplicateVideoModel, isVeo3 } from '@/domain/entity/model'
...
export const buildCreateElements =
  ...
  async ({ chat, plan, settings, platform }) => {
    ...
    videoModels = videoModels
      .filter((model) =>
        model.providers?.length ? model.providers.some((provider) => !provider.disabled) : true,
      )
    ...
    const videoModelElement: IChatSettingsElement = {
      ...
      data: videoModels
        .map((videoModel) => {
          ...
          if (!isAllowed) {
            allowedPlanType =
              plansModels.find(({ model_id }) => model_id === videoModel.id)?.plan.type ?? null
          } else {
          ...
          return {
            ...
            is_default:
              isAllowed &&
              plansModels.some(
                (planModel) => planModel.model_id === videoModel.id && planModel.is_default_model,
              ),
          }
        })
    ...
    const elements = [
      isVeo3(model)
        ? getVeoQualityElements()
        : isReplicateVideoModel(model)
          ? getVeoAspectElements()
          : getRunwayAspectElements(model.id),
      videoModelElement,
    ]

    if (settings.model !== 'veo-3') {
      elements.unshift(
        isReplicateVideoModel(model)
          ? getVeoDurationElement(settings)
          : getRunwayDurationElement(settings),
      )
    }
...
const getVeoQualityElements = (): IChatSettingsElement => {
  return {
    id: 'video_quality',
    code: 'video_quality',
    type: 'string',
    ...
    data: [
      {
        id: 'standard',
        code: 'video_quality_standard',
        label: 'standard',
        value: 'standard',
      },
      {
        id: 'high',
        code: 'video_quality_high',
        label: 'high',
        value: 'high',
      },
    ],
