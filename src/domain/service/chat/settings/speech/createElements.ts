...
import { ChatSettingsCustomType, IChatSettingsElement, IChatSpeechSettings } from '@/domain/entity/chatSettings'
    ...
    return [
      {
        ...
        data: speechModels
          .map((speechModel) => {
            const isAllowed: boolean = planModels.some(({ model_id }) => model_id === speechModel.id)

            let allowedPlanType: PlanType | null
            if (!isAllowed) {
              allowedPlanType = plansModels.find(({ model_id }) => model_id === speechModel.id)?.plan.type ?? null
            } else {
            ...
            return {
              ...
              is_default: isAllowed && plansModels.some((planModel) => planModel.model_id === speechModel.id && planModel.is_default_model),
              features: speechModel.features
            }
          })
          .sort((speechModelA, speechModelB) => (speechModelA.order ?? 0) - (speechModelB.order ?? 0))
          .sort((speechModelA, speechModelB) => Number(!speechModelA.is_allowed) - Number(!speechModelB.is_allowed))
      },
      {
        id: 'speech_voice',
        code: 'speech_voice',
