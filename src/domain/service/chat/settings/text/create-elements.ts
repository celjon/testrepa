          ...
          data: textModels
            .map((textModel) => {
              const isAllowed: boolean = planModels.some(
                ({ model_id }) => model_id === textModel.id,
              )

              let allowedPlanType: PlanType | null
              if (!isAllowed) {
                allowedPlanType =
                  plansModels.find(({ model_id }) => model_id === textModel.id)?.plan.type ?? null
              } else {
              ...
              return {
                ...
                is_default:
                  ...
                  plansModels.some(
                    (planModel) =>
                      planModel.model_id === textModel.id && planModel.is_default_model,
                  ),
              }
            })
            .sort(
              (textModelA, textModelB) =>
                Number(!textModelA.is_allowed) - Number(!textModelB.is_allowed),
            ),
        },
