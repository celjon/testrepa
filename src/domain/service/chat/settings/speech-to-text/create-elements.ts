...
import { ChatSettingsCustomType, IChatSettingsElement } from '@/domain/entity/chat-settings'
      ...
      data: STTModels.map((STTModel) => {
        ...
        return {
          ...
          is_default:
            isAllowed &&
            plansModels.some(
              (planModel) => planModel.model_id === STTModel.id && planModel.is_default_model,
            ),
        ...
        .sort(
          (STTModelA, STTModelB) => Number(!STTModelA.is_allowed) - Number(!STTModelB.is_allowed),
        ),
    }
