...
export const buildAddModel = ({ planModelRepository, planRepository, modelRepository }: Adapter): AddModel => {
  return async ({ type, modelId }) => {
    ...
    const upsertPlanModel = async (plan: IPlan, modelId: string, disabledTelegram: boolean) => {
      ...
      if (planModel.model.children && planModel.model.children.length > 0) {
        await Promise.all(planModel.model.children.map((childModel) => upsertPlanModel(plan, childModel.id, false)))
      }
    }

    await Promise.all(plans.map((plan) => upsertPlanModel(plan, modelId, !isMidjourney({ id: modelId }))))
