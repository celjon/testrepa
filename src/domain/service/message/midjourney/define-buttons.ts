...
import {
  getMessageButtonActionByMJNativeButton,
  IMessageButton,
} from '@/domain/entity/message-button'
...
export const buildDefineButtonsAndImages = ({
  ...
  return async ({ generationResult, messageId, disabledButtons = ['❤️'] }) => {
    ...
    if (
      ['U1', 'U2', 'U3', 'U4', 'V1', 'V2', 'V3', 'V4'].every((label) =>
        buttons.some((button) => button.label.includes(label)),
      )
    ) {
      ...
      const { width: mjImageWidth = 1024, height: mjImageHeight = 1024 } =
