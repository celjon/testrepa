...
import { DefineError, ErrorType } from './define-error'
...
const updateAccountGenerations = async (
...
const incrementModelFunctionUsage = async (
...
const buildErrorMessage = (errorMessage: string, imagineParams: ProcessMjParams): string => {
  ...
  return (
    ...
    (message.images?.length
      ? `Ссылки: ${message.images.map((image: IMessageImage) => image.original?.path).join(',\n ')}\n`
      : '') +
...
): Promise<ModelAccountStatus | undefined> => {
  let messageToSend = ''
  let status: ModelAccountStatus | undefined
...
export const buildProcessMj = ({
  ...
  const processMj = async ({
    ...
    try {
      ...
      if (
        imagineParams.modelFunction === 'imagine' &&
        !url &&
        imagineParams.callback &&
        imagineParams.settings
      ) {
        ...
        imagineParams.modelFunction === 'button' &&
        imagineParams.button &&
        imagineParams.callback
      ) {
      ...
      if (errorType !== ErrorType.INVALID_PROMPT && errorType !== ErrorType.MODERATION_FAILED) {
        const status = await handleErrorNotification(
          errorType,
          account,
          errorMessage,
          imagineParams,
