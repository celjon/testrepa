...
import { IModelProvider, isG4FProvider, isOpenAIProvider } from '@/domain/entity/modelProvider'
...
export const buildSendImageByProvider = ({ dalleGateway, g4fGateway, modelProviderRepository, modelService }: Params) => {
...
const buildSendByFallbackProviderOrThrow = ({ sendByProvider }: { sendByProvider: SendImageByProvider }) => {
  const sendByFallbackProviderOrThrow: SendImageByFallbackProvider = async ({ provider, model, error, ...params }) => {
    const logEntry = {
    ...
    if (provider.fallback && !provider.fallback.disabled) {
      try {
        ...
        logger.error(`sendByFallbackProviderOrThrow[image] Fallback provider ${provider.fallback_id} failed to send message`, { error: e })
