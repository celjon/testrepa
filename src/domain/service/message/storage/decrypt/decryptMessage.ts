...
import { decryptImages, decryptMedia } from '@/domain/service/message/storage/decrypt/decryptFiles'
    ...
    if (message.isEncrypted) {
      ...
      if (Array.isArray(message.search_results)) {
        message.search_results = await Promise.all(
          message.search_results?.map(async (searchResult: ISearchResult): Promise<ISearchResult> => {
            return {
              ...searchResult,
