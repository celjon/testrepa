...
export const buildEnterpriseUsageModerationService = (
  params: Adapter,
  ...
  return {
    moderate: async ({ enterpriseId, userMessage }) => {
      ...
      let prompt =
        'Call this function if any of messages violates listed rules. \n User is not allowed to:'
      ...
      if (
        tool_calls &&
        tool_calls.some((el) => el.function.name === 'alert_enterprise_rules_violation')
      ) {
        return {
          flagged: true,
        }
