...
import { ModelProvider } from '@/domain/service/model/getModelProvidets'
import { UseCaseParams } from '@/domain/usecase/types'

export type GetModelProviders = (params: { id: string; sort: string; sortDirection: string }) => Promise<ModelProvider[]>
...
export const buildGetModelProviders =
  ...
  async ({ id, sort, sortDirection }) => {
    ...
    const modelProviders = await service.model.getModelProviders({ author: model.prefix.slice(0, -1), slug: model.id })
