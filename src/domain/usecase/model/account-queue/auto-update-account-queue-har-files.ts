...
export type AutoUpdateAccountQueueHARFiles = (params: {
  accountQueueId: string
  withHARFileUpdatedBefore?: Date
}) => Promise<{
...
export const buildAutoUpdateAccountQueueHARFiles =
  ...
  async ({ accountQueueId, withHARFileUpdatedBefore }) => {
    let accounts = await adapter.modelAccountRepository.list({
      where: {
        queue_id: accountQueueId,
        g4f_har_file_updated_at: withHARFileUpdatedBefore
          ? {
              lte: withHARFileUpdatedBefore,
            }
          : undefined,
      },
    })
        ...
        try {
          ...
          if (eventData.type === 'HAR_FILE_UPDATED') {
            const account = accounts.find(
              (acc) => acc.g4f_email && acc.g4f_email === eventData.email,
            )
            ...
            if (account) {
              ...
              logger.info({
                ...
                message: `Successfully updated HAR file for ${account.name} `,
