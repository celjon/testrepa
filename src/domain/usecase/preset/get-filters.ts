import { IPresetFilter } from '@/domain/entity/presetFilter'
...
export const buildGetFilters =
  ...
  async ({ locale = 'en' }) => {
    ...
    for (const categoryFilter of categoriesFiltersAll) {
      ...
      const uniqueCategoryFilter: IPresetFilter | null = categoriesFiltersMap.get(categoryFilter.category.code) ?? null
    ...
    const categoriesFilters: IPresetFilter[] = [...categoriesFiltersMap.values()].filter((categoryFilter) => categoryFilter.item_count > 0)
